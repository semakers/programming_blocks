...
class Draggable<T extends Object> extends StatefulWidget {
  ...
  const Draggable({
    ...
    this.feedbackOffsets = const [Offset.zero],
  ...
  final List<Offset> feedbackOffsets;
...
class _DraggableState<T extends Object> extends State<Draggable<T>> {
  ...
  _DragAvatar<T>? _startDrag(Offset position) {
    ...
    final _DragAvatar<T> avatar = _DragAvatar<T>(
      ...
      feedbackOffsets: widget.feedbackOffsets,
...
class _DragAvatar<T extends Object> extends Drag {
  ...
  void updateDrag(Offset globalPosition) {
    ...
    final List<List<_DragTargetState<Object>>> parallelTargets = [];

    for (var feedbackOffset in feedbackOffsets) {
      final HitTestResult result = HitTestResult();
      WidgetsBinding.instance.hitTest(result, globalPosition + feedbackOffset);
      parallelTargets.add(_getDragTargets(result.path).toList());
    }

    final List<_DragTargetState<Object>> targets =
        parallelTargets.reduce((a, b) => a.length > b.length ? a : b);
